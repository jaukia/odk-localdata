...
def parse_as_dict(svg, cached=False, _copy=True): 
    ...
    """Returning a dictionary. Not reliable yet
    """ 
    #paths_dict={}
    #for i in range(len(ids)):
    #   paths_dict[ids[i]] = paths[i]
    #
    #return paths_dict
...
def parse_node_id(node, paths=[], pids=[]):
    ...
    if node.hasChildNodes():
        for child in node.childNodes:
            paths_pids = parse_node_id(child, paths, pids)
            paths = paths_pids[0]
            pids = paths_pids[1]
    ...
    if node.nodeType == node.ELEMENT_NODE:
        if node.tagName == "g":
            pids.append(parse_id(node))
      #  else: pids.append("no_id")
        #try:
        if node.tagName == "line":
            paths.append(parse_line(node))
        if node.tagName == "rect":
